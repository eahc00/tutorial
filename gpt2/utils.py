...
def top_k_top_p_filtering(
    ...
    if top_k > 0:
        top_k = min(max(top_k, min_tokens_to_keep), logits.size(-1))  # 최소 토큰 개수 보장
...
def generate_custom(
    ...
    for _ in range(max_length):
        with torch.no_grad():
            outputs = self.hg_model(input_ids=generated_ids)  
    ...
    generated_text = self.tokenizer.decode(generated_ids.squeeze(), skip_special_tokens=True)
